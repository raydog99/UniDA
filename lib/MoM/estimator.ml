...
let median_block blocks =
  let values = List.map (fun block -> Tensor.mean block ~dim:[0] ~keepdim:false) blocks in
  let sorted = List.sort Tensor.compare values in
  List.nth sorted (List.length sorted / 2)

let clip_weights w c =
  Tensor.clamp w ~min:(Float.neg c) ~max:c

let estimate x y k_x k_y learning_rate n_iter =
  let c = 0.01 in (* clipping parameter *)
